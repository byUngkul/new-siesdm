...
class Sumur_model extends CI_Model{
	...
	public function tampil(){
		$this->db->select('t_sumur.id_sumur, t_sumur.no_sumur, t_sumur.no_sipa, t_sumur.tgl_ahir_sipa, t_sumur.lokasi_sumur, t_sumur.debit_izin, t_sumur.no_seri_pompa, t_sumur.no_meter_air, t_sumur.sipa, t_sumur.peta_lok, t_sumur.tera_meter, t_sumur.hasil_uji_air, t_sumur.id_zona, t_perusahaan.nama_perusahaan, t_kota.nama_kota, t_jenis_sumur.jenis_sumur, DATEDIFF(t_sumur.tgl_ahir_sipa, CURRENT_DATE()) as selisih');
		...
		$this->db->order_by('id_sumur', 'DESC');
	...
	public function getEndSipa($param = '', $wilayah = ''){
		if (!empty($wilayah)) {
			if ($param == '1') {
				$this->db->select('t_sumur.id_sumur, t_sumur.no_sumur, t_sumur.no_sipa, t_sumur.tgl_ahir_sipa, t_sumur.lokasi_sumur, t_sumur.debit_izin, t_sumur.no_seri_pompa, t_sumur.no_meter_air, t_sumur.sipa, t_sumur.peta_lok, t_sumur.tera_meter, t_sumur.hasil_uji_air, t_sumur.id_zona, t_perusahaan.nama_perusahaan, t_kota.nama_kota, t_jenis_sumur.jenis_sumur, DATEDIFF(t_sumur.tgl_ahir_sipa, CURRENT_DATE()) as selisih');
				$this->db->from('t_sumur');
	...
	public function add(){ // tambah data sumur
		...
		if (count($post) > 0) {
				$data = array(
					...
					'hasil_uji_air'   => 
							$this->_upload('./uploads/uji_air/', 'pdf|doc|docx|jpg|jpeg|png|zip', 'UJI_'.$val, 'hasil_uji_air'),
